...
EXP_DIR = "experiments/scaling/collision_3m"
CP_DIR = "checkpoints/scaling/collision_3m"
...
def main():
    ...
    print(f"""========================================
COLLISION-3M PHASE 10 TRAINING RUN
========================================
...
""")
    ...
    # Save experiments/scaling/collision_3m/config.json
    exp_config = {
        "experiment_name": "collision_3m_scaling",
    ...
    initial_cp_path = os.path.join(CP_DIR, "collision-3m-initial.pt")
    save_checkpoint(
    ...
    while not finished_training:
        for x, y in train_loader:
            ...
            if step % 500 == 0:
                ...
                cp_name = f"collision-3m-step-{step:06d}.pt"
                ...
                if val_loss < best_val_loss:
                    ...
                    best_step = step
                    best_checkpoint_path = os.path.join(CP_DIR, "collision-3m-best.pt")
                    save_checkpoint(
                        model, optimizer, scheduler, step, 0, avg_train_loss_for_record, val_loss,
    ...
    plt.title("COLLISION-3M Loss Curve")
    plt.xlabel("Step")
    plt.ylabel("Loss")
    plt.grid(True)
    plt.legend()
    ...
    # 1.46M Baseline details
    base_params = 1462464
    base_val_loss = 1.9363
    base_perp = 6.93
    # Look up Phase 6 benchmarks or training logs for comparison
    # Total training time for Phase 6 was ~490.5 seconds as reported in report.md
    base_train_time = 490.5
    # Look up Phase 6 baseline inference speed from Phase 6 benchmark results if available.
    # Phase 6 inference speed was around ~120 - 150 tokens/sec
    # Let's run a quick dry-run benchmark of the 1.46M model to verify actual speed on the current platform
    ...
    param_inc = ((param_count - base_params) / base_params) * 100
    loss_chg = ((best_val_loss - base_val_loss) / base_val_loss) * 100
    perp_chg = ((np.exp(best_val_loss) - base_perp) / base_perp) * 100
    time_chg = ((total_train_time - base_train_time) / base_train_time) * 100
    speed_chg = ((avg_inf_speed - base_inf_speed) / base_inf_speed) * 100
    
    # Save results.md
    results_path = os.path.join(EXP_DIR, "results.md")
    with open(results_path, "w", encoding="utf-8") as f:
        f.write(f"""# COLLISION-3M Scaling Experiment Results
...
| :--- | :---: | :---: | :---: |
| **Model Parameters** | {base_params:,} | {param_count:,} | {param_inc:+.2f}% |
| **Best Validation Loss** | {base_val_loss:.4f} | {best_val_loss:.4f} | {loss_chg:+.2f}% |
| **Best Validation Perplexity** | {base_perp:.2f} | {np.exp(best_val_loss):.2f} | {perp_chg:+.2f}% |
| **Total Training Time (s)** | {base_train_time:.1f}s | {total_train_time:.1f}s | {time_chg:+.2f}% |
| **Avg Inference Speed (tok/s)** | {base_inf_speed:.2f} | {avg_inf_speed:.2f} | {speed_chg:+.2f}% |

## Observations
- **Loss Improvement**: Validation Loss changed from `{base_val_loss:.4f}` to `{best_val_loss:.4f}` ({loss_chg:+.2f}%).
- **Perplexity Change**: Validation Perplexity changed from `{base_perp:.2f}` to `{np.exp(best_val_loss):.2f}` ({perp_chg:+.2f}%).
- **Training Efficiency**: Parameter count increased by `{param_inc:.1f}%` resulting in a `{time_chg:+.1f}%` training time change on CPU.
- **Inference Speed Change**: Generative throughput changed from `{base_inf_speed:.2f}` tok/s to `{avg_inf_speed:.2f}` tok/s ({speed_chg:+.2f}%).
