"""
COLLISION Ultra Frontier Scaling Benchmark (250M -> 1.0 Billion Parameters).

Explores the absolute upper scalability limits across 50 training optimization steps per tier.
Measures parameter counts, step latency, memory consumption, loss drop, and hardware limits.
"""
...
# Remove data script dir from sys.path if present
...
ULTRA_TIERS = [
    {
        "name": "COLLISION-350M (Heavy Frontier)",
        "d_model": 1152,
        "n_layer": 24,
        "n_head": 18,
        "d_ff": 3456,
    ...
    {
        "name": "COLLISION-500M (Half-Billion Anchor)",
        "d_model": 1280,
        "n_layer": 28,
        "n_head": 20,
        "d_ff": 3840,
    ...
    {
        "name": "COLLISION-750M (High-Density Frontier)",
        "d_model": 1536,
        "n_layer": 28,
        "n_head": 24,
        "d_ff": 4608,
    ...
    {
        "name": "COLLISION-1.0B (Billion Parameter Frontier)",
        "d_model": 2048,
        "n_layer": 24,
        "n_head": 16,
        "d_ff": 5376,
...
def benchmark_ultra_tier(tier: Dict[str, Any], steps: int = 50, batch_size: int = 2, device: str = "cpu") -> Dict[str, Any]:
    print(f"\n=======================================================", flush=True)
    ...
    print(f"    -> Exact Parameter Count: {total_params:,} ({total_params / 1e6:.2f}M / {total_params / 1e9:.3f}B)", flush=True)
    ...
    est_hours_10m_gpu = est_hours_10m_cpu / 35.0  # GPU acceleration factor on tensor cores

    result = {
        "name": tier["name"],
        "parameters": total_params,
        "params_m": round(total_params / 1e6, 2),
        "params_b": round(total_params / 1e9, 3),
        "d_model": tier["d_model"],
        "n_layer": tier["n_layer"],
        "n_head": tier["n_head"],
        "d_ff": tier["d_ff"],
    ...
    print(f"    => Tier Done: Avg Latency: {avg_latency_ms:.1f}ms | Loss Drop: {losses[0]:.4f} -> {losses[-1]:.4f} (Delta -{losses[0]-losses[-1]:.4f}) | Est 10M on GPU: {est_hours_10m_gpu:.2f} hrs\n", flush=True)
    return result


def main():
    print("=================================================================", flush=True)
    print("COLLISION ULTRA FRONTIER SCALING BENCHMARK (250M -> 1.0B+)", flush=True)
    print("=================================================================", flush=True)

    results = []
    for tier in ULTRA_TIERS:
        res = benchmark_ultra_tier(tier, steps=50, batch_size=2, device="cpu")
        results.append(res)

    out_dir = os.path.join(PROJECT_ROOT, "experiments", "scaling_benchmark")
    os.makedirs(out_dir, exist_ok=True)
    out_file = os.path.join(out_dir, "ultra_scaling_results.json")
    
    with open(out_file, "w", encoding="utf-8") as f:
        json.dump(results, f, indent=2)

    print(f"=================================================================", flush=True)
    print(f"All ultra-frontier tiers completed! Saved to {out_file}", flush=True)
    print(f"=================================================================", flush=True)
